/**
 * `$EDITOR`-based row editing. The cursor row is serialized to a TSV temp file
 * and handed to the user's editor (`$EDITOR`/`$VISUAL`, falling back to `vi`),
 * spawned inline in the terminal. While the editor owns the terminal the
 * renderer is suspended (raw mode off, alternate screen released), then
 * resumed on exit; the edited file is parsed back and offered to the caller as
 * typed UPDATE parameters.
 *
...
import { parseRowTsv, serializeRowToTsv } from "@/editor/row-serialization"
...
/** Launch `$EDITOR` over a temp TSV of the row and report parsed changes. */
export const editRowInEditor = (options: EditRowOptions): void => {
  ...
  const file = join(dir, `${safeName(tableName)}.tsv`)
  let wrote = false
  try {
    writeFileSync(file, serializeRowToTsv(columns, row), "utf8")
    wrote = true
  } catch (cause) {
  ...
  child.on("exit", code => {
    ...
    clean()
    const result = parseRowTsv(edited, columns, primaryKey, row)
    if (!result.ok) {
